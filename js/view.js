export class View {
  ...
  elements = {
    productList: document.querySelector(".catalog__card-list"),
    ...
    // form: getElementById("aside-form"),
  };
...
form.addEventListener("submit", (e) => {
  ...
  filterProducts(form); // передаём форму
});
...
function getFilterValues(form) {
  ...
  const powdered = [
    ...form.querySelectorAll('input[name="powdered"]:checked'),
  ].map((el) => el.value);
  // Получение типов
  ...
  return { minPrice, maxPrice, materials, powdered, types };
}

// Фильтрация товаров
function filterProducts(products, form) {
  const { minPrice, maxPrice, materials, powdered, types } =
  ...
  const filtered = products.filter((product) => {
    ...
    const matchesMaterial = materials.length
      ...
      : true;
    const matchesPowdered = powdered.length
      ? powdered.includes(product.powdered)
      : true;
    const matchesType = types.length ? types.includes(product.type) : true;
    console.log("matchesMaterial", matchesMaterial);

    return matchesPrice && matchesMaterial && matchesPowdered && matchesType;
  });
...
function renderProducts(arrData) {
  ...
  arrData.forEach((product) => {
    const li = document.createElement("li");
    li.className = "catalog__card-item";
    li.innerHTML = getProductMarkup(product); // Функция, возвращающая HTML-разметку товара
    fragment.appendChild(li);
  });
